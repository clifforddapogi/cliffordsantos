...
function UnderlineField({ label, name, type = "text", required, value, onChange, testid, area = false }) {
  const Tag = area ? "textarea" : "input";
  const props = area ? { rows: 4 } : { type };
  return (
    <label className="block">
      ...
      <Tag
        ...
        className={`input-underline ${area ? "resize-none" : ""}`}
        data-testid={testid}
      />
    </label>
  );
...
function ContactInfoRow({ k, v, href }) {
  const inner = (
  ...
  return (
    <div className="col-span-12 lg:col-span-5">
      ...
      <div className="mt-10 space-y-4">
        <ContactInfoRow k="Email" v={site?.email} href={`mailto:${site?.email}`} />
        <ContactInfoRow k="Phone" v={site?.phone} href={`tel:${site?.phone?.replace(/\D/g, "")}`} />
        <ContactInfoRow k="Location" v={site?.location} />
        <ContactInfoRow k="LinkedIn" v="ca.linkedin.com/in/cliffordsantos" href={site?.socials?.linkedin} />
      </div>
    </div>
  );
}
...
function StatusLine({ status }) {
  ...
  if (status.state === "error") {
    return <span className="text-red-600" data-testid="contact-error">{status.error}</span>;
  }
  return null;
}
  ...
  return (
    ...
    >
      <div className="space-y-2">
        <UnderlineField label="Name" name="name" required value={form.name} onChange={setField} testid="contact-name" />
        <UnderlineField label="Email" name="email" type="email" required value={form.email} onChange={setField} testid="contact-email" />
        <UnderlineField label="Phone (optional)" name="phone" value={form.phone} onChange={setField} testid="contact-phone" />
        <UnderlineField label="Tell me about the project" name="message" area required value={form.message} onChange={setField} testid="contact-message" />
      </div>

      <div className="mt-8 flex items-center justify-between flex-wrap gap-4">
        <div className="text-sm text-ink/60"><StatusLine status={status} /></div>
        <button
          type="submit"
