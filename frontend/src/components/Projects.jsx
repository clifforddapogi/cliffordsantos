...
function ProjectCardOverlay({ p, inverted }) {
  ...
  return (
    <div className={`absolute inset-0 flex flex-col justify-between p-6 lg:p-8 transition-opacity duration-500 ${overlayBg} opacity-0 group-hover:opacity-100`}>
      <div className="flex items-start justify-between">
        <span className="text-[10px] uppercase tracking-[0.22em] font-bold text-cyan_brand">{p.category}</span>
        <ArrowUpRight className={`w-5 h-5 ${titleColor}`} />
      </div>
      <div>
        <h3 className={`font-display font-bold tracking-tight text-2xl lg:text-3xl ${titleColor}`}>{p.title}</h3>
...
function ProjectCardLabel({ p, inverted }) {
  ...
  return (
    <div className={`absolute inset-x-0 bottom-0 px-5 pt-12 pb-5 lg:px-6 lg:pb-6 group-hover:opacity-0 transition-opacity ${labelBg}`}>
      <div className="text-[10px] uppercase tracking-[0.22em] font-bold text-cyan_brand">{p.category}</div>
      <div className={`font-display font-bold tracking-tight text-xl lg:text-2xl ${titleColor}`}>{p.title}</div>
    </div>
  );
}
...
function ProjectCard({ project: p, idx, onOpen }) {
  ...
  return (
    ...
    >
      <ProjectCardImage src={p.image_url} alt={p.title} />
      <ProjectCardOverlay p={p} inverted={inverted} />
      <ProjectCardLabel p={p} inverted={inverted} />
    </motion.button>
  );
}

function ProjectModalDetails({ project }) {
  return (
    <div className="p-8 lg:p-12">
      <div className="text-[11px] uppercase tracking-[0.22em] text-cyan_brand font-bold mb-3">{project.category}</div>
      <h3 className="font-display font-bold text-navy text-3xl lg:text-5xl tracking-tightest leading-none">{project.title}</h3>
      <p className="mt-6 text-lg text-ink/80 leading-relaxed">{project.description}</p>
...
function DetailItem({ label, value }) {
  return (
    <div>
      <div className="text-[10px] uppercase tracking-[0.22em] text-ink/50 font-bold mb-1">{label}</div>
